...
3. One can experiment on a model in a way that may be impractical or unethical for a real system\footnote{This idea is the premise of the humour in the sketch from \emph{That Mitchell and Webb Look} quoted at the beginning of the chapter.}}.
...
Other types of mathematical models include simulation models, and optimisation models (the latter is also known as mathematical programming). Both of these use equations, inequalities and logical dependencies to represent the properties of a system. A simulation predicts how a system will perform under a given set of conditions, for example could be the length of a traffic queue given a particular system of traffic signals [@Grether2012].

Optimization models compute the conditions under which a system will perform best (with 'best' defined by the modeller). An example would be to work out how traffic signals should be timed in order to minimise traffic congestion [@Osorio2008].

Both simulation and optimisation models use sets of equations formed to represent the physics of a system and the real-world constraints applied to it. These equations are formed of parameters, which are fixed values representing unchanging realities of the system ((such as the length of a given road); and variables, which are allowed to change (such as the length of time for a signal to display a green light). In simulation models, variables 

Whereas optimisation models 

Mathematical routines operate on the equations to choose the variable values which minimise the of interest.[TODO:]

The reverse of each other 
In these models, one can test scenarios, such as seeing how queue lengths changes when commuters alter their departure times.
