...
In particular,
a systems engineering approach should be introduced to unify
and standardize the methods of categorizing and quantifying
data; to establish databases that will support investigation,
analysis, and evaluation; to obtain multiscale data to meet the
Environmental Science & Technology Critical Review
DOI: 10.1021/acs.est.5b03060
Environ. Sci. Technol. 2015, 49, 11247−11263
11256
database requirements of such analyses; to improve our
understanding of urban metabolic principles and scale effects
to support management and policy development and meet the
requirements of different stakeholders; to integrate the insights
from different fields of research; and to design research that will
provide solutions for specific social policy problems, which will
guide industrial development and that will promote restoration
of the urban ecological balance.

### Summary

Returning to consider the broader aims of the thesis [TODO: continue] 
...
This section seeks to move this idea forward by considering the role modelling can play. First, the concept of modelling generally is introduced, before taking a systematic look at modelling within urban metabolism.

### An introduction to modelling

Whenever models are used, it is important to justify their use, but it is especially important here specifically, as we introduce a new model to the urban metabolism field.

A model can be defines as
...
Note here the explicit reference to 'system' in the above definition. It is precisely the complexity of a system -- a concept introduced back in Chapter 1 -- with its various interactions and moving parts, which warrants simplified representations, i.e. models.
...
Models are therefore not a 'magic bullet', but nonetheless they are still important as "one of a number of tools for decision making" [@Williams1999, p5] to test different options, and identify possible courses of action.

### Types of models

Models come in different types, which are summarised in Figure \ref{fig:models} (which is based on the @Williams1999's brief introduction to models). The first categorisation divides models into those that are *concrete* or *abstract*. The former type are exist physically, for example, a scaled-down construction of an aeroplane or laboratory-scale version of some resource management process). The latter type exist as conceptualised representations of a system.
...
Abstract models can also be *mathematical*. These use mathematical relationships to represent the properties of a system. One particular type of mathematical models are statistical models. In these, the statistical properties of a set of sample data (for example, the relationships between the age and height of 50 children selected randomly from a school) are used to predict the characteristics of a larger group (for example, the height, if the age of any child in the school is known).

Other types of mathematical models include simulation models, and optimisation models (the latter is also known as mathematical programming, which was briefly introduced in Section [TODO:]). Both of these use equations, inequalities and logical dependencies to represent the properties of a system. A simulation predicts how a system will perform under a given set of conditions, for example could be the length of a traffic queue given a particular system of traffic signals [@Grether2012]. Optimization models compute the conditions under which a system will perform best (with 'best' defined by the modeller). An example would be to work out how traffic signals should be timed in order to minimise traffic congestion [@Osorio2008].

The equations of simulation and optimisation models represent the physics of a system and the real-world constraints applied to it. These equations are formed of parameters, which are fixed values representing unchanging realities of the system ((such as the length of a given road); and variables, which are allowed to change (such as the length of time for a signal to display a green light). In simulation models, the variable values are selected by the modeller, who wants to investigate the effect of their changes; whereas for optimisation models, the reverse is true -- the desired effect is specified first, and the variables to achieve this are chosen by by mathematical routines.

[TODO: Move to a summary section.]

Simulation and optimisation models need to be based on descriptive models: what a descriptive model visualises, a mathematical model can formalise into mathematics, with the relationship between system components being expressed mathematically. A descriptive model has already in fact been proposed in this chapter, namely the hierarchical framework of Figure \ref{fig:metab}. The object of modelling in this work is to optimise the system -- the network of processes and resources -- for any given sets of demands and available resources
