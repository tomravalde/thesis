...
More specifically, this thesis will focus on the energy provision, water consumption and waste management which sustain a city's population. As will be argued later, this leads to both economic and environmental concerns. In addressing these, there are many complementary approaches which planners and policy makers might take. Consider a city whose electricity partly supplied by a coal-fuelled power plant, using the resulting heat energy to generate steam which rotates a turbine to generate electricity which is then delivered to the consumer by cable (Figure \ref{fig:elec}). One way to save coal is to encourage consumers to use less electricity. Another option is to increase power plant efficiencies, so as to minimise energy losses within the plant. Alternatively, the power plant could be replaced by a process using renewable resources, such as solar. While interventions such as these have their place, this thesis will zoom out to look at how resource management works as a whole. Is the overall mix of power plants, water treatment facilities, waste management procedures, and other activities, the most efficient way to manage resources?
...
Before considering the economic and environmental challenges of urban resource management, there are two important facts to note. The first is that urbanisation is not going to stop anytime soon -- it is inevitable.  At present, urban areas are home to just over half of the global population [@UN2014]. By 2030, this figure is expected to grow to 60 per cent, and to over 65 per cent by 2050 (Figure \ref{fig:pop_forecasts}). This growth has three components: cities absorbing population growth; migration from rural areas; and reclassification of rural areas as urban [@Cohen2006]. These predictions are not just the view of a small number, but widely accepted opinion. Thus even @Cohen2004 who takes a skeptical view of the projections (pointing out that historical forecasts can be overestimates by more than 75 per cent, and that there is no universally consistent definition of 'urban'[^urban-def]) concludes:
...
The inevitability of urbanisation ultimately derives from its desirablity -- this is the second fact to note: urbanisation should not by stopped anytime soon. Urbanisation is the longterm result of humanity's search for the good life. People migrate from the countryside in search of employment, as well as better access to education, healthcare, and other services. Thus an urban population grows. As it does, firms benefit from economies of scale, and at the same time, cities start becoming 'agglomeration economies' (in which different types of firms and industries benefit one another by way of supplying each other with goods and services, on account of their proximity to one another). With scale and agglomeration, cities can specialise in the goods they produce and the services they supply. These goods and services can then be traded which increases prosperity, which in turn attracts migrants. Thus urbanisation becomes a virtuous cycle in which city size and economic growth feed one another (Figure \ref{fig:urbanCycle}) [@WorldBank2009].
...
The environment is a huge field of study, and to summarise all negative environmental effects arising from the way that cities use resources would be impossible. However, just a few examples builds a compelling case that levels of resource use by urban areas is problematic enough to warrant attention from researches, planners, and policy makers. To give some sense of order to the environmental challenges, they will be listed in relation to the resource 'sectors' which cause them, first as problems which are manifestations from a specific sector, and second as problems which emerge from the interactions of two or more sectors. This notion of sectors will become important an theme of the thesis[^sectors].
...
The meeting point of the above issues is climate change, which is arguably the greatest environmental threat of all [@WEF2016]. The energy and waste sectors are large contributers of greenhouse gas emissions, and climate change limits the availability of water in many places [@AlFry2006]. Thus the three sectors are linked by climate change. However, not only do the energy, water and waste sectoral strains share a point of contact with climate change, they also interact with one another; in other words, demand in one sector can feed the problems of another. The effect of these 'intersectoral interactions' (introduced below, and summarised in Figure \ref{fig:intersectoral-interactions}) is to exacerbate the environmental challenges of individual sectors listed above.
...
The second sustainability challenge brought about by rates of urban resource consumption is economic. @Bettencourt2007a have demonstrated that the populations of certain types of city (namely, those cities whose economies are 'knowledge-based') can grow a rate that is impossible to sustain with the available resources. This fact is derived from two equations which model features of urban growth. First, at time $t$, quantities in cities $Y(t)$ (such as energy consumption or wealth), scale with population, $N(t)$, according to a power law:
<!--
...
In a second equation of urban growth, population growth is related to the resources required to sustain it. If each individual requires some quantity of 'resources', $R$, to maintain them per unit time, and 'extra' resources, $E$, to accommodate a new member, then the total resource need, $Y$, is given by:
...
The detailed mathematics which describe the origin of these scaling laws from first principles is presented in @Bettencourt2013. The mathematics is rather involved, but the efficiency inherent to urban areas can be explained simply, with one word -- *proximity*. A city facilitates the mixing of people such that the socioeconomic gains surpass the resource demands. This thesis will look particularly at the infrastructural proximity, and how (as mentioned earlier), its geographical co-location means cities might be well placed to be the solution to their own problem. Specifically, where management processes are close both to one another, and to consumers who have a diverse mix of resource management demands, there are a couple of strategies which can be exploited to use resources more efficiently.
...
A second strategy is **cascading** -- another way to improve energy system efficiency. The classic example is CHP, in which the waste heat from power generation is used to provide useful heat energy \citep{Grubler2009}. While the water management sector does not use the term 'cascading', the same principle can still be applied, namely using waste products of one process, as the input to another. Often, this is when wastewater from one purpose is used for another purpose whose quality standards are less stringent. An example for a household could be the separation of potable and nonpotable water streams, such that grey water from the washing could be used in toilet flushing or watering plants \citep{TheGreenAge2016}.

### Intersectoral synergies

Both decentralisation and cascading are are possible because of the proximity of infrastructure to resource feeds, consumers and to each other. The examples listed above show how cities can get the most out of a particular infrastructure system. However, these efficiencies are not necessarily the limit of what can be achieved. In cities, infrastructural proximity can also apply to the infrastructure of *different* sectors. In theory, this allows the waste and by-products of one sector to become the inputs to another. In other words, resources can be cascaded between sectors (and this can be facilitated by the decentralisation of processes). So not only are there the intersectoral *strains* mentioned above, but the same sectoral pairings also present opportunities for intersectoral *synergies*.
...
This methodology is used extensively in urban resource management, though it tends to be applied to individual sectors such as energy systems, or water distribution, but (as the literature review in Chapter 2 will show), current models do not consider the different urban resource management systems together in such a way as to take advantage of intersectoral synergies as in Figure \ref{fig:broader-system} [^optimisation-examples]. (Another way of saying this is that some nodes have multiple functions: a power plant power plant is not just considered part of the electricity network, but also as part of the water network because of its need for cooling water.) Nevertheless, existing models could be a helpful starting point for the development of a model which plans the integrated management of energy, water, and waste in urban areas.
...
In summary, this thesis aims to focuses on the engineering possibilities of urban resource management challenges, in sufficient detail to be useful to researchers, planners and policy makers, but without becoming lost in an unbounded set of political, social, and economic realities.
...
This end goal is made up of three aims, which combine to address the overall research question:
...
- Following from the above item, this thesis contributes a new open-source database of resource management processes which records their various features, including the quantities of resources they consume and produce, their rates of operation, and other properties (Chapter 4).
- To formulate a model which simultaneously optimises energy, water and waste management processes at the urban scale (Chapter 5). The development of this model will be described in some detail given its novelty to the field. Development will be done in the context of a 'benchmark study', which others can use to develop and improve the model presented here.
...
The above contributions will be realised over the seven chapters of this thesis. A diagram of how the chapters build upon one another is given in Figure \ref{fig:thesis-structure}, and a summary of each chapter's objectives and contribution, and how they relate to each other, are given in Table \ref{tab:outline}.

This chapter has argued that urbanisation is both a socioeconomically desirable phenomenon, and yet also a source of environmental and economic problems stemming from resource overconsumption, but that cities possess the solution to their own problem, if only they were to take full advantage of the resource integration opportunities inherent in them. Chapter 2 examines the literature to show how this work advances the urban metabolism concept, and develops existing models which optimise resource management.
