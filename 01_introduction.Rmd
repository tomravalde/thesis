...
There is a precedent for exploiting intersectoral synergies in this way, which is found in the field of 'industrial symbiosis' (IS). This is where firms use each others' waste outputs as inputs in other processes [@WRAP2016]. Often, this is realised in 'eco-industrial parks' (EIPs) in which industries, business and consumers are co-located with the deliberate intention of practising IS [@PresidentsCouncil1996]. The classic EIP case-study is Kalundborg, a town in Denmark. A key component of Kalundborg's economy are industrial production facilities and agriculture: these engage in a set of mutually beneficial material and energy exchanges with each other, and with the local energy system \citep{Lewis2000}. Exchanges include a power station which provides its waste steam to an oil refinery (which in turn produces gas for the power station), its waste heat to fish culture, and ash to cement manufacture; the fish culture provides sludge used by local farmers, who also use sludge from a bioplant fed by steam from the power station. These and other exchanges are shown in Figure \ref{fig:kalundborg}.
...
Researchers have been able to quantify some of the benefits of Kalundborg's IS. To cite just one finding; analysis suggests that Kalundborg has avoided extracting four million gallons per day of freshwater, by treating and reusing effluent from wastewater treatment [TODO: cite http://pubs.acs.org/doi/full/10.1021/es050050%2B]. What is interesting about Kalundborg is that these benefits came about as intersectoral synergies evolved over time through the initiative of private firms -- they were not centrally planned by planners and policy makers [@Desrochers2001]. However, this thesis is concerned with actively helping planners and policy makers realise intersectoral synergies, and not just in a few specific industrial production facilities, but more generally in urban resource management. But just a little reflection identifies the technical challenge. If a city contains a large number of resource management processes, located in different places, then how should they adopt resource-saving strategies in the best way possible, and all this without breaking any financial, practical or other constraints)?
...
From this node-link formalisation, one can formulate a mathematical representation of a system, in which the behaviour of processes (e.g. the quantities of resource the consume and produce), and other system properties are described by equations. In other words, it is possible mathematically *model* urban resource management. By applying appropriate algorithms to the mathematical model, it is possible to compute the optimal mix of processes and the schedule of resource transfers between them, using a technique known as *optimisation modelling*.

This methodology is used extensively in urban resource management, though it tends to be applied to individual sectors such as energy systems, or water distribution, but (as the literature review in Chapter 2 will show), current models do not consider the different resource management systems together in such a way as to take advantage of intersectoral synergies[^optimisation-examples]. Nevertheless, these (and other) precedent models could be a helpful starting point for the development of a model which plans the integrated management of energy, water, and waste in urban areas. A contribution of this thesis will be to attribute nodes with  multiple functions. Thus a power plant is not just considered part of the electricity network, but also as part of the water network (because of its need for cooling water). These links and others are shown in Figure \ref{fig:broader-system} which incorporates the electricity network.

[^optimisation-examples]: Examples are given in Section \\ref{sec:rev-um-opt-models}.
...
Any discourse on the sustainability of urban resource management should interact with the field of 'urban metabolism' (UM). UM -- whose history and methodology will be considered in more detail in Chapter 2 -- proposes that cities and living organisms are in some ways analagous: both entities consume resources, and produce wastes, and in doing so, sustain their activities and growth [@Kennedy2011]. This analogy offers a 'metaphorical framework' [@Pincetl2012, p.194] which provides the concepts and language to aid the understanding of the relationship between a city and its resources, both the flows into, within, and out of a city, and the stocks held inside a city over time. The field is broad and flexible, and patterns of resource use can be related to many aspects of a city, including its form and geography, socioeconomic indicators, political and institutional arrangements [@Hobbes2007], and its activities and functions.

Some key findings of UM studies show that in general there is an increasing per capita consumption of water, energy and materials, and output of waste and wastewater \citep{Kennedy2008}, and that this growth of resource flows is coupled to growing GDP \citep{Schulz2007a, Kowalski1997}. Nevertheless, for some cities, there are observed per capita reductions in energy and water use (through increased efficiency) and reduced per capita waste consumption (following increased recycling rates) \citep{Kennedy2008}. Some UM studies take a zoomed-out perspective, for example showing that as human settlements grow (from hunter-gather, to agricultural communities, to industrial centres), they become dependent for their resources on an ever-widening surrounding area \citep{Haberl2001a, Haberl2001b, Barles2009}. Other studies zoom in on the detail: for example, considering exactly how a city becomes water-stressed through the different stages of urban development \citep{Kennedy2008}.

The metaphor has proved to carry lots of explanatory power. Indeed, the challenges set out in this thesis have already been defined by others in terms of UM. For example, many have proposed that cities should move from having a linear metabolism (where resources are consumed and expelled by a city with little or no reuse or recycling within the city) to a circular metabolism (where co-located activities use one-anothers' wastes and by-products) [@Leduc2010, @Leduc2013, @VillarroelWalker2014, @Leidreiter2012], as represented in Figures \ref{fig:linear-met} and \ref{fig:circ-met}. The UM concept is increasingly advocated in the design and planning of urban areas, and even finding its way into urban planning policy [@Agudelo-Vera2011, @Agudelo-Vera2012] and education, with @Kennedy2011 advocating UM thinking to generate "more ecologically sensitive designs" of a city through infrastructure integration, closing loops to minimise overall inputs and outputs of resources. In summary, the UM concept is key to improving resource management by bringing resource sustainability considerations to urban planning.
...
This thesis is built on the case that the urbanisation patterns seen around the world should be embraced because of the quality of life it brings to more and more people, but there are associated environmental and economic sustainability issues that need to be tackled. However, due to the co-location of their infrastructure, cities may have inherent to them the ability to address these challenges in someway by encouraging synergies between energy, water and waste management sectors. The overall aim of this would be to improve an urban area's metabolism. To realise these benefits in practice is no small challenge -- not least because of the difficulty of choosing which mix of processes will result in the overall best set of synergies. However, this type of problem is well-suited to optimisation modelling, in which the urban resource management networks are formalised as a system, which is mathematically represented in such a way that optimisation techniques can find the optimal configuration of energy, water and waste management infrastructure.


### Definition of thesis title and research boundaries {#sec:title-def}

Urban resource consumption is connected to a lot of different issues and considerations, and thus to provide a focussed are of research, where clear novel contributions can be proposed, requires that research boundaries be defined, in order to pose a precise research question. One way to do this is to provide focus and clarity to the definition of the thesis title. Of the many fascinating aspects of cities and some terms used can have vague, unbounded meanings (such as 'resources', 'processes' and 'sector', amongst others). Defining the boundaries of the research will make it possible to precisely define a research question.

\begin{description}

\item [Highly integrated] This term refers to resource management optimisation models that consider the simultaneous optimisation of more than one system, in order to take into account interactions and synergies between them. (Section \ref{sec:optim-model-overview} defines this more thoroughly and provides examples.) 

\item [urban] There are various ways to define `urban'. \citet{Ramaswami2011} proposes three, which are adapted by \citet{Keirstead2012b}\footnote{Ramaswami's original purpose was to quantify a city's greenhouse gas emissions.}:
...
With its focus on resource management infrastructure, the interest of this thesis is essentially engineering focussed, rather than politically or economically focussed. In particular, the way that a set of co-located consumers (i.e. a city) uses infrastructure to deliver goods and services. Thus the `geographic-plus' definition is most appropriate. In this way, infrastructure located outside a city's administrative boundaries, but is proximate to other infrastructure, would be considered as part of the urban resource management system.
...
As well as the research boundaries identified by the title definition, there are boundaries to the research worth noting. With respect to the optimisation modelling itself, there will be no discussion on the detailed mathematics of optimisation algorithms, and there will be no attempt to either soft- or hard-link this model to other tools. Considering the application of the modelling, there will be little consideration of how modelling results could be implemented within the real-world, with the social/management aspects to urban resource challenges beyond the scope of this thesis. For the same reason, this thesis considers how modelling affects urban resource flows, but not its effect on other aspects of UM, such as a city's economy, livability, or governance.
...
2. *Develop a model which computes the optimal mix of resource management processes to meet demand for goods and services for an urban area.* These models must integrate the energy, water and waste systems, be computationally tractable, and provide results in a form useful to researchers, planners and policy makers. This will involve developing test models which will be applied to a small-scale case study to provide a 'benchmark' case study, which can be used to assess the performance of models (in terms of tractability, ease of writing, etc.)
...
This thesis makes five novel contributions to research:

- To advance the UM concept by considering how resource management processes affect an area's overall metabolic flows. The need for this contribution is discussed in Section \ref{sec:procs-in-UM}. This will help those interested in urban sustainability think about it in a new way, as well as providing the concepts behind the mathematical formulation of the systems optimisation model.
- In this cause of the above item, this thesis contributes a new open-source database of resource management processes which gives definitions of the quantities of resources that processes consume and produce, their rates of operation, and other properties (Chapter 4).
- To formulate an optimisation model which simultaneously optimises energy, water and waste management processes at the urban scale (Chapter 5). The development of this model will be described in some detail given its novelty to the field. This model development will be done in the context of a 'benchmark study', which others can use to develop and improve the model presented here.
- To comment on the appropriate set of metrics (Chapter 3) to quantify how well an area's metabolism is performing. These metrics could be used to assess an area's metabolism as well as the results of the modelling work. 
- To provide a new tool which brings together systems optimisation and urban metabolism, to help planners and policy makers incorporate UM thinking into sustainable urban design and planning.
...
The next three chapters contribute the work that is needed to develop and apply the model. Chapter 3 explores the different ways one could quantify the efficiency of urban metabolism. Chapter 4 systematically researches all the processes that can be used to manage energy, water and waste in the short- to mid-term, to provide data for the model (which is also provided in the open-source database). Chapter 5 formulates three possible optimisation models, and applies them to the benchmark study to establish the most suitable way to formulate the highly integrated urban energy, water, and waste management systems optimisation model.
...
tab.tex <- xtable(tab.csv, caption="The relationships between research objectives, thesis chapters and novel contributions.", label = "tab:outline", align="llp{6cm}p{5cm}p{6cm}")
...
%	\end{spacing}
	\caption{Thesis outline and the relationship between chapters [TODO: Complete figure].}
