...
\epigraph{``I don't know what London's coming to — the higher the buildings the lower the morals.''}{Noel Coward, \emph{Collected Sketches and Lyrics}}
...
Using political instruments to address the large climate issues is familiar territory [TODO: complete]. Over recent decades, a united will to stop damazge to the ozone layer meant [TODO: N] counties banned CFCs with the result that [TODO: continue]. In more recent decades, climate change has been a constant target of policy, with nations coming together at Kyoto and Paris [TODO: provide further details] to set emissions targets [TODO: etc.] 
...
1. *Scale economy driven growth.* As mentioned previously, in this case $\beta<1$, resulting in sigmoidal growth, such that the population will tend to a finite value.
2. \label{itm:agglom} *Agglomeration economy driven growth.* In this case, knowledge and innovation are the growth drivers, such that $\beta>1$.

	a. *Resources aren't at their limit.* $N$ will grow with $t$ at a greater-than-exponential rate, theoretically reaching an infinite population in a finite time, $t_c$:
\begin{align*}
	t_c&=-\frac{E}{(\beta-1)R}\ln\left[1-\frac{R}{Y_0}N^{1-\beta}(0)\right] \\
	&\approx \left[\frac{E}{(\beta-1)R}\right]\frac{1}{N^{\beta-1}(0)}.
\end{align*}
	b. \label{itm:collapse} *Resources become limited.* However, since resources are limited, then there will come a time when they run out and stagnation and collapse occurs. 
...
Note the difference between these issues:

- economic felt within a city
- environmental strain effects distributed more widely (less possible to calculate)
...
- Headline stats, that for the same quality of life, cities are more efficient.
