...
This section gives the key particulars on how the sets and parameters above are defined from the SPC documentation, with the data summarised in Table [TODO: cross-ref]. To tabulate all the resource demands for 
...
We will assume that the three broader regions will be linked via a connection running north-south through the site. Then we assume that the functions within a particular region are equidistant from the region's centre (with the exception of the energy island which does not belong to a region). With the connections determined, the distances are estimated, based on the site's dimensions. We assume that the region-to-region connections join the regional centroids, thus distances can be measured by scaling distances from the site plans in the SCP documentation. The region-to-function distances will be assumed as [TODO:] km.
