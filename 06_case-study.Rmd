...
```{r summarise_metabolisms, fig.height=7, fig.width=7.5, fig.cap="Counting the resources in metabolic flows into and out of the SPC site suggests that making more processes available to the model results in a more diverse mix of aggregate metabolic flows.\\label{fig:metabolism_summary}"}
## Summarise numbers of processes

flows_summary <- metabolic_flows %>% distinct(scenario, season, resource) %>% select(scenario, season, resource) %>% group_by(scenario, season) %>% summarise(Resources = n())

plot_flows_summary <- ggplot(flows_summary, aes(Resources, scenario)) +
  geom_point(aes(colour=season), position=position_dodge(width=0.6)) +
#  facet_wrap(~Flow, ncol=1) +
  labs(colour="Season") +
  xlab("Number of resources in aggregate metabolic flows into or out of SPC site") +
  theme_bw() +
  theme(axis.text.x=element_text(hjust=1, angle=30, size=6.5),
	axis.title.y=element_blank())
plot_flows_summary
...
In all scenarios, energy conversion processes are dominant, but there are nevertheless intersectoral interactions, such as the energy demands in water and wastewater treatment, and synergies, such as heat generated by the watersource heatpump in the Design case scenarios (Figures \ref{fig:designCase_minCost} and \ref{fig:designCase_minWater}). In each scenario, the system middle (i.e. the network of conversion processes and resource flows) differ, according to the design objective and available processes.
...
The design case scenarios have up to twelve unique processes available to use. Each scenario has chosen a common set of seven processes: (1) water treatment plants, (2) wastewater treatment plants, (3) an anaerobic digestion facility (which generates gas from the organic fraction of waste), (4) ground-source heat pumps, (5) water-source heatpumps, and (6) absorption chillers, and (7) heat exchanges (to deliver hot and cold water through a district heating network). Overall, the schedule of imports exhibit only one minor difference between the subscenarios, namely no summer import of electricity in the minimum waste scenario, due to [TODO: identify reason].

As well as processes in common, there are differences in the mix of conversion processes (which also explain why the aggregate metabolic outputs vary between the sub-scenarios). For example, the minimum emissions scenario does not include CHP or domestic gas-fuelled boilers (both of which burn fossil fuels), which enables this scenario to achieve zero emissions (Figure \ref{fig:metabolic-flows}). All other scenarios use the CHP to provide much of the heat. However, the minimum-cost scenario joins the emissions scenario in avoiding the gas-fuelled boilers, but this time as a cost-saving measure (though with the side effect of lowering imports of natural gas). However, the waste-minimisation scenario does use gas-fuelled boilers to reduce the need for CHP use which produces fly-ash produced by the CHP, and the water-minimisation scenario [TODO: check these results, complete explanation, rerun models with corrected data if necessary!].
