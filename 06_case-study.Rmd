...
The design case scenarios have up to twelve unique processes available to use. Each scenario uses common set of seven processes: water and wastewater treatment plants, an anaerobic digestion facility (which generates gas from the organic fraction of waste), ground-source and water-source heatpumps, and absorption chilling and heating facilities (which are connected to energy conversion technologies, and deliver [TODO: better word] hot and cold water through a pipe network (which also includes heat exchangers). The four scenarios then have additional conversion processes, according to their objective.

For example, the minimum emissions scenario does not include CHP or domestic gas-fuelled boilers (both of which burn fossil fuels), and as such, the schedule of resource outputs displays zero emissions for this scenario (Figure [TODO: cross-ref]). All other scenarios use the CHP to provide much of the heat. However, the minimum cost scenario joins the emissions scenario in not using the gas-fuelled boilers as a cost-saving measure. However, the waste-minimisation scenario uses them to reduce the fly-ash produced by the CHP, and the water-minimisation scenario [TODO: check these results, complete explanation, rerun models with corrected data if necessary!] To minimise waste, the waste-minimisation scenario, the anaerobic digestion plant plays a greater role.

[Note on scope 1/2/3 emissions.]
...
The wildcard scenarios each have the same objective of cost minimisation, and share [TODO: N] processes in common. However, there is some small variation in both the overall metabolic flows, and the process mix. With regards to the inflows, inputs of fossil fuels (coal and gasoline) are a notable variation: the current scenario imports them in all three seasons, whereas the medium scenario imports them in to winter only, and the longterm scenario imports them in winter and summer. The longterm scenario also has a summer inflow of algae. [TODO: explain these differences with reference to the networks.] 
...
The results above reveal that the metabolic flows are genuinely affected by the network of management processes, thus justifying this thesis. This means that optimal networks are sensitive to the objective, i.e. the AD/boiler synergies does not exist int the minimum water design case scenario (though the energy island does work well). When different objectives are applied to the same technology choice (e.g. 2a and 2b [TODO: update if necessary]), we observe the sensitivity of the technology mix in the network [TODO: can this be quantified by some metric?].
...
However, what is the effect of PRaQ's optimisation on headline metrics?
