...
```{r summarise_metabolisms, fig.height=7, fig.width=7.5, fig.cap="Making more processes available results in the model choosing more management processes and a more diverse mix of aggregate metabolic flows into and out of the SPC site.\\label{fig:metabolism_summary}"}
## Summarise numbers of aggregate metabolic flows

flows_summary <- metabolic_flows %>% distinct(scenario, season, resource) %>% select(scenario, season, resource) %>% group_by(scenario, season) %>% summarise(Value = n())
...
process_summary <- process_mix %>% distinct(scenario, season, process) %>% select(scenario, season, process) %>% group_by(scenario, season) %>% summarise(Value = n())
...
The Design Case scenarios have up to twelve unique processes available to use. Each scenario has chosen a common set of seven processes: (1) water treatment plants, (2) wastewater treatment plants, (3) an anaerobic digestion facility (which generates gas from the organic fraction of waste), (4) ground-source heat pumps, (5) water-source heatpumps, and (6) absorption chillers, and (7) heat exchangers (to deliver hot and cold water through a district heating network). Overall, the schedule of imports exhibit only one minor difference between the subscenarios, namely no summer import of electricity in the minimum waste scenario. 

- minimises waste by sending organic waste to the anaerobic digestion
- the resulting gas goes to the CHP to generate heat and electricity
	- the heat then goes to the district heating network (via the heat exchangers) to generate cooling and heating as necessary
	- the electricity goes to HVAC systems for cooling, supplemented by electrical imports
- but shoulder season requires minimum electricity for heating and cooling (zero cooling, and less heating than the winter -- see plot of Demands)
- which means that the CHP generates sufficient electricity to supply the HVAC in shoulder, without the need for imports
