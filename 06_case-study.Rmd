...
System cost is the first headline result, summing the cost of the conversion and transport infrastructure, as well as the cost of importing resources. These costs are calculated on an annual basis, assuming a ten year life span of infrastructure. This can be done using Equation \ref{eq:objective_cost}, $C = \gamma^R C^R + \gamma^P C^P + \gamma^{\tau} C^{\tau}$ with $\gamma^*$ taking values of 1, 0.1, and 0.1, respectively.
...
headlines[headlines$metrics=="headline_emissions", "metrics"] <- "Emissions [kg/year]"
headlines[headlines$metrics=="headline_waste", "metrics"] <- "Waste [kg/year]"
headlines[headlines$metrics=="headline_water", "metrics"] <- "Water [kg/year]"


plot_headlines <- ggplot(headlines, aes(scenario, values)) +
  geom_point() +
...
Turning to consider the metabolism of each scenario in more detail, Figure \ref{fig:metabolic-flows} gives a closer look at the resource imports and exports for each scenario. Note the following:

- The fewer processes available, the simpler the import/export regime
- seasonal variation

Coupling these results to the headline: more resources are used to achieve the headline outcomes.

```{r metabolic_flows, fig.height=7, fig.cap="fig:metabolic-flows", fig.width=6.5, results="asis"}
...
Next we consider the network of conversion processes which makeup each system. These are shown in Figures \ref{fig:designCase_minCost} to \ref{fig:wildcard_long} in plots which show synergies between processes (i.e. their linking due to the transfer of resources between them). This is achieve by using 'social network analysis' (SNA) plots. Note the following features:

- Each process is represented by a circle whose colour indicates the sector it belongs to[^sna-process-type].
- Grey lines connect any pair of processes in which one of the processes uses a resource produced by they other.
- The proximity of any pair of processes is proportional to the degree to which they are dependent on one another. From this, it follows that the more central a process is to the plot, the more its integrated into the network because it tends to be linked to more other processes. Conversely, the closer a process is to the fringe of a plot, the less other processes dependent on this.
- As well as including processes in this diagram, 'imports' and 'exports' are marked (coloured orange); thus if an imported resource is used in any particular process, then there will be a line connecting imports and said process (likewise for exports). The rules of proximity apply here also.
...
Perform a simplified version of the method outlines in Section [TODO: cite Grey-box metrics subsection of Chapter 3], in which there is no need to distribute the flows between the processes within the city, on the understanding that in the model, any process products unused within other processes will become system outputs (exports), thus exergy analysis can be peformed on a whole-system level on a black-box, with the internal 'grey box' analysis implicit to it:
...
Tablulate absolute values and efficiency:

#### Ecological network analysis

Plot graphs for each scenario.
