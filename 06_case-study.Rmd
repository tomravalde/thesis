...
%	\end{spacing}
	\caption{SPC's proposed interactions between energy, water and waste management. Resources in \textbf{bold} represent resources either demanded or produced by the end-users in the regions of Figure \ref{fig:site-connections}; and * indicates a resource can be imported from outside the system boundary (represented by the dashed line). These processes will be split between the zones of the site and be connected by transport infrastructure.}
...
Each of the twelve zones shown in Figure \ref{fig:site} will be represented in the model, and thus each belong to the set of zones $z \in Z$, where $|Z|=12$. It is assumed that the three regions will be linked via a connection running north-south through the site, and also that the functions within a region are equidistant from the region's centre (with the exception of the energy island which does not belong to a region). This makes the size of set $nb_{z,z'} \in ZZ'$ as $|nb|=12$. The connection distances have been estimated from the site's dimensions. Assuming that the region-to-region connections join the regional centroids, distances can be estimated by scaling distances from the site plans in the SPC documentation. The region-to-function distances will be assumed as 0.5 km. In total, there are twelve two-way connections defined, which are given in Table \ref{tab:zonal-connections}. The transport processes used for these connections are detailed in Section \ref{sec:case-study-transport-processes}, but briefly, roads are assumed to exist along any connection. Additionally, these roads have the option to run underground pipes and overhead cables along their length[^road-cost]. The GAMS code for site layout data is listed in Appendix \ref{sec:app-case-study-site}.
...
\begin{table}[]
\centering
\caption{The zonal connections defined for set $nb(z,z')$ (with bold text denoting a broader region), and their corresponding lengths $l(z,z')$ used in the SPC case study.}
...
The resource demands used here are summarised in Figure \ref{fig:site-demands}, showing the seasonal demand for energy (electricity, heating, and cooling), water (potable, and reclaimed), and waste management (wastewater, non-organic solid waste, and organic solid waste). The precise numeric values used in the model are provided in Appendix \ref{sec:app-case-study-resources}. Resource demands are defined as rates, i.e. on a per-second basis (kg/s or MW). This is a common way to express energy quantities, which makes sense given that energy is most commonly demanded on such a basis, i.e. as a power[^energy-rate]. This makes the demand parameter definitions consistent with PRaQ's definition of process behaviour, because process rates are defined on a per-second basis.

[^energy-rate]: This is in contrast to waste management services (to give one example) in which waste may be collected at regular intervals (e.g. weekly), and set to a particular processing unit once a certain threshold (e.g. tons) has been met. Nevertheless, for usability, there should be consistency between the definition of energy and mass quantities. One way to capture the non-rate properties of many material-based processes would be to introduce storage processes into the model. This may be the subject of future developments, as discussed in Section \ref{sec:case-study-further-work}.

### Other resource parameters

The resource demands discussed above will define some of the $r$ items in set $R$, but to complete this set requires the resources which used in the network of conversion processes to meet demand. In total, there are 68 resources, including everything from algae to wood. Each resource has at least one associated quality $q$, which is attributed to the resource using the $X_{rq}$ and $\delta^R_{rq}$ parameters. The qualities used in this case study are *mass* and *energy*. Most resources are only attributed with one of these (e.g. algae possesses mass, and heat possesses energy), though some resources are attributed with both (e.g. ground water has both a mass and an energy head). Each resource is also given a cost, based on values from the literature. The full list of resources along with their associated qualities and costs is given in Appendix \ref{sec:app-case-study-resources}. With the resources, zones and times determined, the remaining resource-related parameters can be defined, namely: limits on how many zones can import and export particular resources ($N^I_r$, $N^E_r$), the maximum allowable imports of a resource in a season ($I^{max}_{rt}$, $E^{max}_{rt}$), and the cost and emissions factors of resources ($c^R_r$ and $\epsilon^R_r$).

### Conversion processes

The contents of the process set $P$ will vary for each scenario, and will draw from both the SPC proposals, and the database of urban metabolism processes outlined in Chapter 4. The information needed to define the processes are the coefficients which define the input and output resource quantities with $k^P_{prq}$, and maximum process rates $F^{P,max}_p$. For the database processes this information already exists, but for the SPC proposals, this information needs to be derived from the SPC documentation.

In addition to these parameters, each process needs a cost, $c^P_p$, for use in the objective function. For this modelling, uses cost estimates. This is not without its difficulties. First, there is the shear number of processes in the database (202 in total). Second, the process database includes processes which have not yet penetrated into the market or are still to be developed, thus there are no values which can be easily lifted from the literature. Third, currency exchange rates and purchasing power parities around the world make it difficult to attribute a cost to a process with any degree of certainty. For this reason, costing is approximate.

The broad approach to estimating process costs is to assemble a number of 'process categories' ($\mbox{PC}$), to which processes from the database -- 'database processes' ($\mbox{DP}$) -- are assigned. As an example, *'biogas-fuelled CHP'* is defined as a $\mbox{PC}$, which includes four types of biogas-fuelled CHP conversion processes in the database. Costs are assigned to the $\mbox{PCs}$, in terms of a process's capacity (i.e. USD/MW or USD/kg/s). These costs are obtained from the literature -- this may require some assumptions and adjustments to the numbers -- details are given in Appendix \ref{sec:app-case-study-processes}). A guiding principle of these estimations is that process costs exhibit economies of scale [^scale-economy]. For this to be captured in the model, each $\mbox{PC}$ is assigned both a lowerbound and an upperbound unit cost. Thus, the total cost of any particular $\mbox{DP}$ with a given capacity (MW or kgs/s) can be calculated from:
...
where $^*$ can indicate either a lowerbound or an upperbound cost ($\mbox{Cost}^{LB}$ and $\mbox{Cost}^{UB}$, respectively). In this way, lowerbound costs are applied to larger instances of a process, and upperbound costs applied to smaller instances, and thus $\mbox{DPs}$ become cheaper as they get larger.

For the conversion processes which do not yet exist in the market, costs are based on existing processes which have the same main resource[^main]. This method is conservative in its approach, because it assumes that new processes will not penetrate the market unless they deliver a given service at lower cost than a more traditional means of conversion[^cost-estimation].
...
Transport processes are defined by the origin-to-destination resource input and output coefficients, $k^{*}_{\tau rq}$ (where $*$ stands for $\alpha$, $\beta}$, $\alpha'$, and $\beta'$), and account for the resource carried (e.g. water through a pipe), as well as the energy resources required to carry that resource along the length of a connection (e.g. electrical energy to pump water along a pipe), as well as any by-products, wastes, and emissions from the process. The main resource typically leaves the origin zone and arrives in the destination (perhaps minus some losses, such as power along a cable), the auxiliary resources and by-products should be appropriately shared shared between the origin and destination. The specifics of transport process coefficients are outlined below, for the three main types of transport technologies, with example values of $k^*_{\tau rq}$ given in Table \ref{tab:transp-coeffs}. 
...
\item[Pipes] These transport $r=\mbox{water}$ or $r=\mbox{gas}$, and require electricity as an auxilliary resource to provide pumping energy. As hydraulic calculations given in Appendix \ref{sec:case-study-site} show, for water in a pipe of zero slope, this energy can be roughly calculated as $0.0054l_{zz'}$ Joules to overcome friction between the water and the pipe\footnote{This assumes the nonlinearities in the relationship between transport mass and volume can be ignored -- further explanation is given in Appendix \ref{sec:case-study-site}}. The modelling assumes that all this electricity is provided in the zone of the water's origin. For gases it is assumed that friction is low enough so as to make the energy requirements negligable for the distances in the SPC site.
...
The idea central to this thesis is that the aggregate metabolic flows into and out of a city are governed to a significant extent by the middle of the system, i.e. a network of resource management processes. These metabolic inputs and outputs are displayed for each scenario in Figure \ref{fig:metabolic-flows}. The first broad observation is that, making more processes available to a system tends to design systems which use a larger number of resource management processes and have a greater number of aggregate metabolic inputs and outputs to and from the site (Figure \ref{fig:metabolism_summary}). Thus PRaQ suggests that a site's resource-management objective is best met by a diverse mix of management processes working with a broad set of resources.
...
The process networks for each scenario are visualised in Figures \ref{fig:designCase_minCost} to \ref{fig:wildcard_long}. These are SNA-style node-link graphs (undirected) in which processes that transfer resources one to another are connected by lines. The proximity of any pair of linked processes is proportional to the quantities of resources transferred. From this, it follows that the more central a process is to the plot, the more integral it is to the network because it is linked to more processes. Conversely, the closer a process is to the plot's fringe, the less other processes depend on it. The processes have been coloured according to the management sector to which they belong[^sna-process-type]; to help visualise the intersectoral interactions and synergies. Inputs, outputs and demands are also represented; to show which processes import and export resources. In each scenario, the system middle differs according to the design objective and available processes. All scenarios are dominated by processes whose main aim is to produce energy, but these nevertheless exhibit intersectoral interactions (such as the energy demands in water and wastewater treatment), and synergies (such as heat generated by the water-source heatpump) in the Design Case scenarios (Figures \ref{fig:designCase_minCost}--\ref{fig:designCase_minWaste}). 
...
The Design Case scenarios have up to twelve unique processes available to use. Each Design Case has chosen a common set of seven processes: (1) water treatment plants, (2) wastewater treatment plants, (3) an anaerobic digestion facility (which generates gas from the organic fraction of waste), (4) ground-source heat pumps, (5) water-source heatpumps, and (6) absorption chillers, (7) heat exchangers (to deliver hot and cold water through a district heating network), and (8) electrically-powered HVAC systems (to provide cooling). As well as the scenarios having processes in common, their different objectives drive differences in the mix of conversion processes selected for the system's middle, affecting the aggregate metabolic flows into and out of the site at the top of the system. These differences are summarised in Table \label{tab:design-case-differences}, and further explained below.
...
The Minimum Waste scenario minimises the site's waste output by using organic waste to feed anaerobic fermentation. This generates natural gas which supplies the both CHP and boiler (which does not happen in the previous two scenarios). In addition, all the wastewater generated by the site is treated (which actually means the site produces more potable water than it needs). This scenario also exhibits seasonal variation, because while the system imports electricity for most of the year, it does not in the summer (Figure \ref{fig:metabolic-flows}). This ultimately relates the demand for heating and cooling, which is met via the district heating network and HVAC systems, which are fed by heat and electricity, respectively. The heat is produced by the CHP, and the electricity is generated by the CHP and supplemented by imports. But these supplementary imports are not required in the shoulder season,  which -- as Figure \ref{fig:site-demands} shows --  has a lower overall total demand for heating and cooling than the rest of the year.
<!--
...
In summary, the results of the Design Case scenarios demonstrate that aggregate metabolic flows are sensitive to design-objectives and do affect the system's middle. Moreover, intersectoral interactions and synergies are important in shaping those networks. This confirms that optimisation modelling serves a purpose in providing insights on how to manage an area's metabolism.
...
Broadly, the results show that exergy efficiency is sensitive to system mix. In general, the scenarios with lower resource imports (indicated by smaller bubbles in Figure ), have lower exergy efficiency. A case in point is the Grid Case, whose low efficiency is mainly driven by the high electricity imports required to meet demand for electricity for end-use and to power air conditioning systems -- these demands cannot be met by the by-products of other processes, and hence result in low efficiency for the system. The Design Cases which minimise emissions and waste are also low exergy efficiency -- this is because in order to minimise these quantities requires using processes which consume other inputs. In other words, there is a trade-off between minimising some quantities and system exergy efficiency.

However, the Minimum Cost scenarios (Design case and Wildcard cases) suggest that high exergy efficiencies can be achieved. This is to be expected if exergy corresponds to the effort required to extract useful work from an energy source, and 'effort' roughly corresponds to cost. Moreover, exergy efficiency seems to increase with a greater number of processes in the system which means there are more places for process by-products to go, to become useful.

#### Ecological network analysis


The other grey-box metric considered by this thesis is ecological network analysis (ENA). Chapter 3 showed this approach could reveal how city's economic sectors work together. The results of this type of analysis are important, because they indicate to planners the extent to which the different management parties rely on one another, and hence where they need to cooperate, and where there are risks/vulnerabilities if there is no high-level understanding of intersectoral dependency (Section \ref{sec:app-grey-box-metrics}). Again, using the method outlined in Section \ref{sec:methods-grey-box}, the interdependencies of management sectors can be quantified in the integral utility matrix, whose elements quantify the combined direct and indirect exergy contributions between any pair of compartments. From this matrix, an overall mutualism index can be computed which quantifies the overall degree of network mutualism [@Zhang2009b]\footnote{$\mbox{Network mutualism} = \frac{\sum \max(\sign(u_{ij}), 0)}{\sum \min(- \sign (u_{ij}), 0)}$} -- the results of this analysis in Table \ref{tab:SPC-ENA} show that network mutualism is higher in the Wildcard cases than the design cases, and highest for the long-term case. This suggest that networks with more processes will ehibit more intersecygtoral synergies -- a result which agrees with intuition. This result taken together with the exergy-analysis and metabolic flow quantities confirms this thesis' hypothesis -- namely that intersectoral synergies bring metabolic efficiency. However, this result also shows the need for the agents who manage each sector to work together, and also highlights the risk of system vulnerability -- if one sub-system (e.g. the energy sector) fails, then the rest of the system may go down with it.
...
This chapter has demonstrated how to apply the PRaQ model to a site using its seasonally and spatially disaggregated demands to plan highly integrated urban resource management systems. Alongside the PRaQ modelling itself, this chapter has described and applied a rule-of-thumb method to estimate the costs of processes, taking into account economies of scale, even for conversion process which do not yet exist.

The results show that the urban resource management systems affect the aggregate metabolic flows into and out of an area, and hence, are sensitive to planning objectives (such as minimising cost, or minimising emissions). Moreover, to achieve an objective, PRaQ will select a large number of processes if they are available (as in the Wildcard Cases). The ecological network analysis show that these larger networks result in the integration of the energy, water and waste sectors. This conclusions come with the fact that meeting certain objectives may sacrifice system effieciency (as measured by exergy).

For the SPC site, the PRaQ modelling justifies the planners' decision to integrate resource management systems, by showing that costs can be minimised, while also keeping exergy efficiency high. The modelling of the design-case scenarios show the SPC's plans to use the energy island and water reuse project are an improvement over direct imports of resources and exports of wastes, and the Wildcard modelling also challenges the planners to consider a more complicated network of processes, but also that this will require the managers of each sector to work together, and to be aware of risks to system failure, should one system go down.

More generally, this case study helps to fulfill Aim 3 of this thesis' Research Question (Section \ref{sec:research-qn}), by showing that environmental objectives can be achieved by optimising an area's process mix, and moreover, by reducing the aggregate metabolic inflows, can also help towards an area's economic security (in relation to the work of @Bettencourt2013). In the language of the conceptual model of Chapter 2, the middle of the system ahas been optimised to achieve the desired effects at the top of the system (Section \ref{sec:procs-in-UM}). The case study shows that integrating sectors to take advantage of synergies can be achieved through a complicated mix of processes -- this demonstrates the power of PRaQ, being able to inform complicated integration strategies that meet an objective, by allowing decision makers to take into account the ever-increasing numbers of technologies which can manage various energy, water, and waste resources (with many more technologies still under development). 

### Further work {#sec:case-study-further-work}

There is further work which could make PRaQ even more useful to decision maker in the SPC project. The first improvements concern data. For example, regarding the choice of processes made available to the model. It may be possible to rule out some processes as not available or feasible for the SPC and/or to decide that the complex wildcard output as a whole is simply infeasible. On the other hand, the SPC decision makers may know of processes not currently in Chapter 4's database which could be included. This both produces more realistic outputs, and speeds up the modelling (in the case that the number of possibilities has been reduced). Other improvements to the data include refining costs, from their current rough estimates, for example, taking into account local prices, and exchanges rates. Thirdly, it would be useful to run PRaQ at finer temporal resolutions (as briefly discussed in Section \ref{sec:case-study-times}) -- this would require more detailed resource demand management data.

The above suggestion of more finely resolved temporal modelling would enable the next suggested area of further work, namely to model the system at an operational level (rather than just the broad design). For the sake of exploratory early-stage planning, this case study has aggregated yearly demand to a single per-second value, however, in reality the process operation rates, inputs and outputs will vary throughout the day. As noted in Section \ref{sec:tat-results}, increasing temporal resolution increases computation time, so this sort of modelling might happen after a technology mix has been chosen, such that the technology choice variables can be fixed. The simplest way to consider operation is to split demand into two periods, average and peak (though finer resolutions could also be chosen). By taking into account demand variation within a day, it will be possible to more appropriately size the system's conversion processes. (Suggestions on how the PRaQ model might handle more time steps were given in Section \ref{sec:formulation-developments}.)

A third type of further work is to alter the objective function used by PRaQ. Examples include incorporating revenue from the selling of exports (this would also increase exergy efficiency as waste exergies are re-defined as useful products, $Ex_p^{prod}$. Other alterations to the objective could be to include environmental taxes on carbon dioxide emissions.

Finally, in parallel to the above three improvements, the SPC planners could apply further sensitivity analysis. This case study has shown how the middle of the system responds to different objectives and the available set of conversion processes. But more sophisticate could look at changing demands (for example, to investigate the effect of population growth, changing consumer behaviour, etc.), the cost of resources, and the cost of processes. In particular, it would be interesting to study a system's resiliance; i.e. how much change it can bear before it becomes over-reliant on importing resources, no longer achieving the metabolic efficiency objectives intended.
