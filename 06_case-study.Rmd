...
The broad approach to estimating process costs is to assign each database process ($DP$) to a 'general process' ($GP$) (e.g. [TODO:]). For each $GP$, a unit cost (USD/MW or USD/kg/s) is obtained from the literature (this may require some assumptions and adjustments to numbers from the literature).
...
In this way, the as yet 

Nevertheless, this is important to get plausible results. (See the supplementary information for references, assumptions, calculations, and other details.)
). 
