...
where $Ex_{j \or i}$ represent the exergy value of a resource (for example the chemical exergy of a fossil fuel, or the exergy of heating, recalling that the reference environmental exergy is considered in this). This method is applied to the scenarios (data and calculations provided in Appendix [TODO: cross-ref]), and the results are given in Table [TODO: cross-ref].
...
names(exergy) <- c("Scenario", "$\\alpha^{in}_{ex}$", "$\\alpha^{waste}_{ex}$", "$\\alpha^{irrev}_{ex}$", "$\\eta_{ex}$")

tab.exergy.tex <- xtable(exergy,
			  caption="Results of exergy analysis for SPC scenarios. All values in MW. \\label{tab:SPC-exergy}",
			  align="llllll")
...
The value of $Ex^{prod}=$ `r ex_prod` for all scenarios.
...
[TODO: Plot graphs for each scenario.]
