# Case study data {#sec:app-case-study}
...
### Site layout {#sec:app-case-study-site}
...
### Resource demands {#sec:app-case-study-resources}
...
### Process options {#sec:app-case-study-processes}
...
Our approach is to assemble a list of general processes ($PC$) for which we can have a unit cost (USD/MW or USD/kg/s) for each process. We can then assign each model process ($MP$) to a $PC$ (indicated by the [TODO: relevant column] column in process-costs.csv). For processes which do not yet exist [TODO: e.g. <example>], we use a GP which produces/consumes a similar main resource. Once each MP has an associated GP Unit Cost, we can estimate the an MP Total Cost (USD) by calculating:
