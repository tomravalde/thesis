...
The `Approaches to urban metabolism' section of the paper was reworked into the literature review [TODO: continue], along with parts of the section headed 'The place of processes in urban metabolism modelling'.
...
The literature review showed the importance of UM as an area of study (especially given the resource management challenges of cities), but pointed out a shortcoming in the field, namely that to date, little attention has been paid to how the mix of resource management processes in the 'middle' of a UM system affect the aggregate exchanges of material and energy with the external environment.

In this Chapter we seek to support process-oriented approaches to UM we identify the resource conversion processes that can shape urban metabolism both now and in the future, and quantify the resource consumption and production of each individual process, assembling this information into a database. We then discuss possible applications of the database, concentrating mainly on how this database can provide data for use in the model to be developed in this thesis, demonstrating intersectoral synergies [TODO: tidy].

## Supporting fields of research

This background introduces two research areas important to the assembly and use of the database: first, we look at databases the currently exist which are similar to ours, and how one may go about assembling; second, we discuss 'open data', particularly in relation to industrial ecology, and how that might bear on the database.
