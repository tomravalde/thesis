...
The network which combines Tat's processes and resources to achieve this is portrayed by the network shown in Figure \ref{fig:tat-16}.
...
networkSummary_csv <- read.csv("05_model-development/network-summary-print.csv", check.names=FALSE)
networkSummary_tex <- xtable(networkSummary_csv, caption="Summary of each network to which the null, PRaQ and nonlinear networks have been applied. \\label{tab:network-summary}", align="lllllp{4cm}")

print(networkSummary_tex,
##      size='\\small',
...
## Applying formulations to Tat Hamlet/Implementation
...
The implications for us are that unless the nonlinear formulation carries with it significant compulational disadvantages. This conclusion is drawn on the basis of having only 16 processes, and so will become even more apposite when recalling that Chapter 4 introduced a library of over [TODO: number] processes. If all these were considered in a model, then computational times could become unrealistic for research purposes.

```{r compare-formulations, fig.height=8, fig.cap="Comparison of model formulations. The plots headed with \textsf{(outliers removed)} show the same information as their counterparts, however with the data for 6 and 8 conversion processes removed. \\label{fig:comparison-formulations}"}
