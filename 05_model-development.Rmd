...
## Why modelling? [NOTE: This section is for the literature review, but we will draw on it in this chapter] {-}
...
### Introduce modelling jargon (so we can refer to it in the review of other models) {-}
...
### Review of energy, water and waste management models {-}

[NOTE: Here would then be the review of energy, water and waste management models, based on the content of the early stage assessment.]

## Introduction 

In the Introduction we argued that cities should take advantage of their capabilities to supply goods and services more efficiently, by taking advantage of the synergies possible between energy, water and waste management processes. We noted that within the energy, water and waste management sectors, optimisation modelling has been successfully used to determine system designs and operational strategies which meet some resource management objective. Thus we proposed that optimisation modelling could be used to manage these sectors in combination, to find ways to minimise the impact of resource consumption, according to some measure. Therefore, in Chapter 2, we defended the use of models in urban decision making, and then reviewed models used in energy, water and waste management, to understand firstly, the capabilities required of our model, and secondly, the mathematical formulation which serves those capabilities.

In this chapter, we want to derive an appropriate formulation to improve an area's metabolism. To get there, we first consider the theory of mathematical modelling to identify the trade-offs inherent in mathematical programming. We also introduce the notion of a 'benchmarking problem' as a problem that can be used to test different models. We introduce the case study which will become our benchmarking problem, and then derive three model formulations to test on the case study. We will then apply the formulations to our case study and present the results, and discuss which formulation will be best for our work.
...
- Describe benchmarking in general, then narrow in on benchmarking within optimistation, and within resource management modelling.

The definition of a 'benchmark' is:

> *"Something whose quality or quantitiy is known and which can therefreo be used as a standard with which other things can be compared".* [TODO: Reference http://dictionary.reverso.net/english-cobuild/benchmark%20problem]

In order to "assess the relative performance of an object" [TODO: http://www.numa.uni-linz.ac.at/Teaching/Bachelor/oberndorfer-bakk.pdf] (e.g. a numerical solver)

This concept of benchmarking can be applied to assessing the "quality of a computational model [TODO: https://books.google.com/books?hl=en&lr=&id=SCUT4QDVkJEC&oi=fnd&pg=PA249&dq=benchmark+problems+AND+water+management&ots=gGB3EhNLBD&sig=qV-], known as 'design benchmarking'. As opposed to 'process benchmarking' which compares the performance of a system. Design benchmarking compares the tools for actually designing/planning the systems by providing "as systematic mechanism for comparing computational tools" (p250) through the use of a test case, on which all computational tools can be applied. For example, [TODO: https://books.google.com/books?hl=en&lr=&id=SCUT4QDVkJEC&oi=fnd&pg=PA249&dq=benchmark+problems+AND+water+management&ots=gGB3EhNLBD&sig=qV-] specify a few problems which can be used to test algorithms which optimise water distribution systems. Some specific benchmarking probelsm include the New York Tunnel system (used since 1969, and studied also in [TODO: http://ascelibrary.org/doi/abs/10.1061/(ASCE)0887-3801(2001)15:2(89)]) [TODO: more examples from other references]. Also, the Anytown network, also considered in [TODO: http://www.iwaponline.com/jh/008/jh0080165.htm], presenting algorithms which produce results of better water quality, at no additional computational cost by adding 'resiliance index' and 'water age' to the already existing cost objective. Adding resiliance to the New York water network problem [TODO: http://www.iwaponline.com/jh/008/jh0080165.htm]

Scopus review of "benchmark problems" OR "benchmarking problem": http://www.scopus.com/term/analyzer.url?sid=BF42418C664C446D2068A80116C54E5A.WeLimyRvBMk2ky9SFKc8Q%3a10&origin=resultslist&src=s&s=TITLE-ABS-KEY%28%22benchmark+problem%22+OR+%22benchmarking+problem%22%29&sort=plf-f&sdt=b&sot=b&sl=60&count=10027&analyzeResults=Analyze+results&txGid=BF42418C664C446D2068A80116C54E5A.WeLimyRvBMk2ky9SFKc8Q%3a19

Particularly common in optimisation problems. They should be: easy to use and complete (so users don't have to go elsewhere for data or instruction). For example, Exeter's online library of benchmark problems [TODO: https://books.google.com/books?hl=en&lr=&id=SCUT4QDVkJEC&oi=fnd&pg=PA249&dq=benchmark+problems+AND+water+management&ots=gGB3EhNLBD&sig=qV-].

Sometimes benchmark problems can be used to compare particular algorithms (e.g. evolutionary algorithms [TODO: http://www.cs.le.ac.uk/people/sy11/Papers/SEAL08_3.pdf], finding optimisation search algorithms that can work effectively and efficiently over a range of problems, using test problems containing "features common to engineering optimization problems" [TODO: http://www.redcedartech.com/pdfs/SHERPA_Benchmark_0110.pdf], problems in mechanics including dynamic systems [TODO: http://iftomm-multibody.org/benchmark/browse/, http://link.springer.com/article/10.1007/s12206-015-0504-4#page-1], finite element problems in structural mechanics [TODO: http://www.sciencedirect.com/science/article/pii/S0168874X0300218X, http://www-ce.engr.ccny.cuny.edu/People/Agrawal/Highway%20Benchmark%20Problem.htm], numerical solutions to Maxwell's equations in electromagnetic problems [http://www.numa.uni-linz.ac.at/Teaching/Bachelor/oberndorfer-bakk.pdf], fluid mechanics [TODO:http://web.mit.edu/kjb/www/Principal_Publications/Benchmark_Problems_for_Incompressible_Fluid_Flows_with_Structural_Interactions.pdf, http://ta.twi.tudelft.nl/nw/users/vuik/software/laplace/problem.pdf], parameter estimation in biological systems found through optimisation [TODO:http://www.ncbi.nlm.nih.gov/pubmed/25880925]

Focussing more within our field, benchmarking problems have been defined for plant production location problems [TODO: http://www.math.nsc.ru/AP/benchmarks/english.html], EAs to optimise water distribution networks [TODO: http://ascelibrary.org/doi/abs/10.1061/(ASCE)WR.1943-5452.0000460], energy storage [TODO: http://castlelab.princeton.edu/Datasets/Time-dependent_storage/readme.pdf, http://castlelab.princeton.edu/Papers/Jiang-ComparisonADPonBenchmarkEnergy-DoesAnythingWorkJune52014.pdf -- note conslusions: no methods suitable, so new theory/methodology is required].

For example, for energy plant operation problems [http://www.ssl.nd.chiba-u.jp/~okamoto/BP-IA/en/P1.html], compared piecewise linear approximations+MILP, search methods, and 'Transformation using ceiling function X DE'; waste collection vehicle routing, testing two types of algorithms, comparing their solutions (objectives variables), computational time, showing which methods finds the preferable solution with speed and the solutions have been successfully used in the real-world.

Note difference between algorithm and formulation
-], 
...
### Tat Hamlet: problem derivation and definition

From the EFA and MFA literature for Tat Hamlet, we are going to define a benchmark problem. This will involve using the values in the literature to define resources and processes, and their demands availability and their demands.

the MFA identifies in Tat, domestic, commercial, agricultural and aquacultural sectors. There is overlap (for example, most houses have their own paddy field, livestock and pond). To simplify the problem, we will ignore the commercial sector (which tends to sell imported final goods, and is hence less interesting for our metabolism problem), and we will assume that the remaining sectors are separate entities. We will take the values of material flows between the sectors to derive:

1. Demands
2. Black box models of processes

Assumptions and simplifications:

- Livestock: many different types, including chickens, dogs, buffalo, and cows. But we will just consider one generic 'livestock' type, and aggregate their inputs and outputs (e.g. water, food, meat and manure, ignoring eggs etc. which in any case are negligible). 
- Agriculture: different types of fields and crops. We will just consider this generically, and aggregate inputs and outputs (e.g. water, manure, green manure, fish feed, firewood and vegetables)
- Aquaculture: fish and water vegetable outputs, from fish feed, manure and fertiliser inputs.
- Domestic inputs include: fire wood, water, vegetables, fish, meat. We also add to this, electricity [TODO: find EFA reference]. Output includes waste.
- Energy assumptions:
  	- Total energy imports = 31.7 GJ/year. We will assume this is split between domestic, agriculture and pumping water (and generated by a dam)
	- Total heat from firewood = 5375.6 GJ/year. Assume this is split between domestic heating, cooking, and domestic hot water
...
[TODO: Note that this section also serves to show how me might start describing any other system we might want to apply this modelling to.]

define a set of demands for the village, and a set of processes which meet those demands. 

Extensions

From the data in the literature, we could represent the Tat network as:


To add more resource demands, and processes (to make the problem more useful as a 'benchmark'), we disaggregate the processes into:
...
- reasoning: small enough to easily understand
	- hence: suitability to become a benchmarking problem
- diagram
- description
- aim

The purpose of this study is to explain why we use our proposed formulation rather than another. It is therefore important that the case study is small enough.

We chose Tat because it is small enough to explain to present an overview of the data in a single paper [TODO: or chapter]. This also makes using our data to replicate or improve on our formulation, user-friendly.


As a case study which we can use to introduce optimisation to the urban metabolism field as a benchmarking problem, we reviewed a number of possible case studies, and concluded that Tat Hamlet would make a suitable case study for a few reasons. First, the literature contains both a material flow analysis [@Hobbes2007, Schandl2006], and an energy flow analysis [@Heezen2004]. Second, Tat is a small subsistent community (105 households and 69 hecatres of farmland) which means the problem is straightforward to comprehend, thus making it accessible as a benchmarking problem (rather than larger, more complicated cases).
...
The first stage of specifying the problem is to decide on the resources and processes that will be modelled. The resources considered are given in Table~\ref{tab:tat_resources}. The processes in the village which consume and produce these resources are:
\begin{itemize}
	\item agriculture,
	\item aquaculture,
	\item cooking,
...
\begin{figure}[h]
	\centering
	\input{05_model-development/tat_network}
...
The calculation of $J$ is somewhat analogous to the calculation of $G$, in that it constitutes a resource coefficient multiplied by a rate, but more sophisticated in several ways ways. First, it is defined for a transport technology $t$ between two zones, $z$ and $z'$, and thus considers each zone both as a source ($src$) from which resources leave, and as a destination ($dest$) into which resources arrive, and thus resource flows arriving in zone $z$ must be summed over all flows coming from all other zones $z'$, and resource flows leaving zone $z$ must be summed over all flows travelling to all other zones $z'$. Second, the amount of resource arriving in/leaving from a zone is a function both of the technology itself and the distance of the technology. For example, [TODO: think up an example, e.g. the amount of electricity used to pump water through a pipe of particular lenght]. This is represented in the bracketed term, which is the multiplier of the transport technology's rate of operation $F^{transp}$ 's variable. Thirdly, it is possible that several transport technologies can carry the same resource (for example biomass can be transported by both lorries or vans: these would have different rates, and require different amounts of petrol etc.), hence the summation over transport technologies $t$. Table [TODO: cross-ref] below gives examples of the parameter values for the examples of electricity and biomass transport in Figure [TODO: cross-ref].
...
Like for conversion processes, $F^{transp}$ is limited by a binary variable which specifies if transport technology $t$ is exists between zones $z$ and $z'$, and must lie between minimum and maximum rates of operation:
...
### Processes, resources and qualitites (PRaQ)
...
- Apply to case study, one process at a time
- Plots of how variables, parameters
  	- Discuss how these relate to generality, fidelity, usability, tractability etc.
- Tabulate key performance characteristics for Null, PRaQ and NL:
 	- Variables ('order' as number of resources/processes, e.g. an approximate mathematical relationship)
 	- Parameters ('order' as number of resources/processes)
 	- Equations ('order' as number of resources/processes)
	- Ease of build
	- Reflectivity of real-world
	- 'Edges' encountered

These can then be discussed in turn
...
\clearpage
\input{05_model-development/tabs/results.tex}
...
\includegraphics[width=\maxwidth]{05_model-development/figure/combo_plot} 

\end{knitrout}

\caption{Plot of null, PRaQ and nonlinear results.}
\label{fig:plot_combo}
\end{figure}

\begin{figure}
\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}
\includegraphics[width=\maxwidth]{05_model-development/figure/null} 

\end{knitrout}

\caption{Plot of null and nonlinear results. This replicates Figure~\ref{fig:plot_combo}, with the omission of zero values.}
\label{fig:plot_null_nl}
\end{figure}
