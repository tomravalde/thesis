...
Not represented by this conceptualisation are the four arbitrarily defined zones. With regards to these, demands are split equally between the zones, and processes can be located in any of them. However, there will be limits imposed which zones can import resources, which will force the model to use transport processes to transfer resources between zones (though there are no restrictions on zones exporting resources). Also not represented are transport processes.
<!--
...
The figure shows that there are seven resource demands for goods and services: water, hot water, electrical appliance use, space heating, meat, fish and vegetables. Demands for water depend upon treatment and pumping processes. Within a household, hot water space heating, and food demands are met by combustion and cooking processes, with food also depending on processes 'upstream' of a house, namely aquaculture, agriculture and livestock, which depend upon water treatment, irrigation, and machinery processes further upstream still. Electricity can only be imported from the grid. Quantitative details regarding demands are given in Appendix [TODO: Table \ref{tab:tat-resources}].
...
Processes in the model developed here will be considered similarly as those in @Keirstead2013a, separated into those that convert resources (such as a wastewater treatment plant), and those that transport them (such as a pipeline). The transport processes connect the different zones into which an urban area is divided, allowing resources to be transported to and from different parts of the city. The main equation is a resource balance,

The model divides the urban area into a number of zones and the time horizon into a number of time periods. 
...
\item [Processes resources and qualities model (PRaQ)] This formulation also linear, and is in many ways similar to the null model, but it develops it to encapsulate a more sophisticated definition of resources, in which a resource can have multiple properties attributed to it. For example, water can have both a mass and an energy head. In practice, this requires defining extra parameters and variables in the model, and inserting some additional equations.
\item [Nonlinear (NL)] This formulation uses nonlinear f
\end{description}


Figure \ref{fig:evolution} shows how the models are related both to the original urban energy systems model, and each other. Each formulation, in theory, increases its fidelity relative to the preceeding model: PRaQ increases in fidelity with respect to resources; and NL increases it with respect to processes too. However, this comes at a cost to generality because the PRaQ and NL formulations require more customisation of the data and equations. [TODO: Is this better in the discussion section? Add comments on]
...
#### An exapmle to demonstrate the differences

The differences between the formulations can be explained with reference to the example of a dam (a conversion process) and a pipe (a transport process).
...
#### Model behaviour and flexibility and generality
...
SUMMARISE WORK/FINDINGS OF CHAPTER
