...
[TODO: Add (qty) superscript, so it doesn't have to be explained in the PRaQ model section]
...
The PRaQ model is based on the same equations as the null model, also following a black-box approach to modelling the conversion processes and transport technologies, and so as before, there is a resource quantity balance (Equation \eqref{praq_balance_qty}:
...
In addition, PRaQ develops Keirstead's formulation, to incorporate a more sophisticated definition of a resource, by using an analogous equation to ensure that the quality $q$ (e.g. energy) due to each resource (e.g. water energy head and generated electrical energy) within each zone $z$ is balanced:
...
The modelling of conversion processes is similar to the black-box formulation of the null model, but again, two equations are used. The first defines the net resource quality $G^{(qual)}_{prqzt}$ produced by a conversion process in a zone at a particular time, for a process with variable operation rate $F_p$, and parameters to define the relative ratio of resource inputs and outputs, $k^{proc}_{prq}$:

\begin{align}
G^{(qual)}_{prqzt} &= k_{prq} F_pzt,
\label{eq:conv_qual}
\end{align}

The second equation is the quantity version of the above, which again uses the $X_{rq}$ parameter, to translate a quantity we can formulate the quantity version of the process black box:
...
- especially when traded off against the other benefits (<< put this in the conclusion)
[<<< TODO: Move to conclusion]

[TODO: Move to conclusion/further work >>>]
- Were this to undergo a full validation, one might 
most of the effort of this type of work will go into the data collection to formulate the black-box models
 (this has been proposed in 'further work')

reliant on the measurements of upstream city inputs being at least as accurate as the model
[<<< TODO: Move to conclusion/further work]
...
#### The formulations in practice: tractability and usability
...
[TODO: Plot just the linear solutions times separately, to (hopefully) show that their run time is linear, possibly scatter them against variables/equations]
...
The choice of formulation is first and foremost guided by the model's purpose -- it is this which provides the degrees of importance given to the four characteristics.
