...
As for conversion processes, $F^{\mathcal{T}}_{\tau z'z}$ is contingent upon a binary variable which specifies if transport technology $t$ is exists between zones $z$ and $z'$, and must lie between minimum and maximum rates of operation:
...
One thing that the developments above cannot do is deal with nonlinear process behaviour. This is not to say that over the longterm, there is no hope for a nonlinear formulation (especially given that computations speeds are becoming greater all the time). Two possibilities present themselves:
