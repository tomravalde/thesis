...
This chapter has developed three formulations of a model which can optimise urban resource consumptions, with varying degrees of fidelity, generality, tractability and usability (\ref{sec:model-concepts}). This section discusses two questions:

1. How well do the Null, PRaQ and Nonlinear formulations perform according to 

Based on the work of this chapter thus far, this section quantifies how well the model performs according to each feature of a model in order to recommend which model should be carried forward for use in a case study.

This question interacts which the one about which of the features are the most important?

Section  identified four features of models. The first two, fidelity and generality, correspond to the way a model represents the real world.
...
This means that assessing the fidelity of this chapter's formulations can only be achieved theoretically. The point has already been made in Section \ref{sec:derivations} that the nonlinear formulation is the most faithful to reality (because it can model phenomena that linear models cannot, such as energy loss due to friction in pipes), followed by the PRaQ model (which defines resources in a nuanced way), following by the Null model.
