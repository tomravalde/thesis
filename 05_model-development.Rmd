...
\begin{align}
I_{rz} + \sum_{\tau}^{\mathcal{T}}J_{\tau rz} + \sum_{p}^{P}G_{prz} &= D_{rz} + E_{rz} \qquad \forall r,z,p,\tau
...
\begin{align}
	F^P_{pz} \leq N^P_{pz} F^{P, \mbox{max}}_{p} \qquad 
...
\multirow{2}{*}{$\tau$}  & \multirow{2}{*}{$r$} & \multicolumn{2}{l}{Source}               & \multicolumn{2}{l}{Destination}                  & \multirow{2}{*}{Comment}               \\ \cmidrule(lr){3-6}
                      &                      & $k^{\alpha}_{tr}$ & $k^{\beta}_{tr}$ & $k^{\alpha '}_{tr}$ & $k^{\beta '}_{tr}$ &                                        \\ 
\midrule
...
\begin{align}
F^{\mathcal{T}}_{tzz'} &\geq \delta^{\mathcal{T}}_{tzz'} F^{\mathcal{T},min}_t \label{eq:null_transp_min} \\
F^{\mathcal{T}}_{tzz'} &\leq \delta^{\mathcal{T}}_{tzz'} F^{\mathcal{T},max}_t \label{eq:null_transp_max}
\end{align}

and transport technologies can only exist between zones which are defined as being neighbours:

\begin{align}
\delta^{\mathcal{T}}_{tzz'} = 0, \qquad \forall (z,z') \notin nb \label{eq:null_neighbs}
\end{align}
...
\begin{align}
\delta^{\mathcal{T}}_{tzz'} = \delta^{\mathcal{T}}_{tz'z} \label{eq:null_direction}
\end{align}
...
\begin{align}
I_{rz} + \sum_{\tau}^{\mathcal{T}} J^{(qty)}_{\tau rz} + \sum_{p}^{P}G^{(qty)}_{prz} &= D_{rz} + E_{rz} \qquad \forall r,z,p,\tau
...
\begin{align}
X_{rq} I_{rz} + \sum_{\tau}^{\mathcal{T}}J^{(qual)}_{\tau rqz} + \sum_p^P G^{(qual)}_{prqz} &= D^{(qual)}_{rqz} + X_{rq} E_{rz} \qquad \forall p,\tau,r,q,z,
...
\begin{align}
X_{rq}V^{(qty)}_{\tau rzt} &= \Big[ \sum_{z'} (k^{\alpha}_{\tau rq} + k^{\beta}_{\tau rq} l_{zz'}) F^{\mathcal{T}}_{\tau zz't} \\
...
[^binary-transp]: As for the conversion processes, $\delta^{\mathcal{T}}_{prq}$ takes a value for 1 for all resource-quality pairings which exist (i.e. $\delta^{\mathcal{T}}_{prq}~\forall p$ when $\delta^R_{rq}=1$).


The existence of transport processes between zones are defined using the same equations as for the Null model:

\begin{align}
F^{\mathcal{T}}_{\tau zz'} &\geq \delta^{\mathcal{T}}_{t zz'} F^{\mathcal{T},min}_t \label{eq:praq_transp_min} \\
F^{\mathcal{T}}_{\tau zz'} &\leq \delta^{\mathcal{T}}_{t zz'} F^{\mathcal{T},max}_t \label{eq:praq_transp_max} \\
