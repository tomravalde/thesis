...
The other process coefficients are then defined relative to this electricity output using Equation \ref{eq:damEnergy}. Given that the quantity of water required to generate a given amount of electricity ($m$) is a variable, for a model to remain linear, the water's elevation head (i.e. the dam height, $z_B$) cannot be considered a variable, and must be a fixed parameter. Therefore, a linear model requires a dam to be defined with fixed height. Thus, assuming a dam of height $z_B=11$ metres and efficiency $\eta^{dam}=$ 80 percent, the mass of water required to produce a megajoule of electricity is 12,742 kg. The dam example has two 'water' resources (one high-head as an input to the black-box model, the other low-head as an output from the black-box model). Other water resources could include water at different temperatures or contamination levels. 
