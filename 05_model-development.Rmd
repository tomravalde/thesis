...
%	\end{spacing}
	\caption{Complete network of the resources and sixteen conversion processes in the Tat Hamlet case study. Resource demands are showin in \textbf{bold}, while resources which can be imported from outside of the system are indicated by a *. Details of transport processes are provided in Table [TODO:].}
	\label{fig:tat-16}
\end{figure}
...
\begin{figure}
	...
	\end{spacing}
	\caption{An example to illustrate the difference between model formulations. There are two zones, with an electricity demand in zone two, which can be met by a dam (conversion process), also in zone two. However, the water must first be pumped (in zone 1), and transported from zone one to two via a pipe (transport process).}
